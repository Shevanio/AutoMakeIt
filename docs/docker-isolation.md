...
This guide covers running AutoMakeIt in a fully isolated Docker container. For background on why isolation matters, see the [Security Disclaimer](../DISCLAIMER.md).
...
3. **Access AutoMakeIt** at `http://localhost:3007`
...
| Can't access web UI   | Verify container is running with `docker ps \| grep automakeit`                               |
| Need a fresh start    | Run `docker-compose down && docker volume rm automakeit-data && docker-compose up -d --build` |
