...
This document describes how context files work in AutoMakeIt and how to use them in agent prompts.

## Overview

Context files are user-defined documents stored in `.automakeit/context/` that provide project-specific rules, conventions, and guidelines for AI agents. They are automatically loaded and prepended to agent prompts.

## Directory Structure

```
{projectPath}/.automakeit/context/
...
The `loadContextFiles` function from `@automakeit/utils` provides a unified way to load context files:

```typescript
import { loadContextFiles } from '@automakeit/utils';
...
**Path:** `/path/to/project/.automakeit/context/CLAUDE.md`
...
**Path:** `/path/to/project/.automakeit/context/CODE_QUALITY.md`
