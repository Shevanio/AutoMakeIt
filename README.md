...
services:
  server:
    volumes:
      # Linux/macOS
      - ~/.claude:/home/automakeit/.claude
      # Windows
      - C:/Users/YourName/.claude:/home/automakeit/.claude
```
...
services:
  server:
    volumes:
      ...
      # Linux/macOS
      - ~/.config/gh:/home/automakeit/.config/gh
      # Windows
      - 'C:/Users/YourName/AppData/Roaming/GitHub CLI:/home/automakeit/.config/gh'

      # Mount git config for user identity (name, email)
      - ~/.gitconfig:/home/automakeit/.gitconfig:ro
    environment:
...
- 💾 **Auto-save** - All work automatically persisted to `.automakeit/` directory
...
- **@automakeit/types** - Shared TypeScript definitions
- **@automakeit/utils** - Logging, error handling, image processing
- **@automakeit/prompts** - AI prompt templates
- **@automakeit/platform** - Path management and security
- **@automakeit/model-resolver** - Claude model alias resolution
- **@automakeit/dependency-resolver** - Feature dependency ordering
- **@automakeit/git-utils** - Git operations and worktree management
...
- **File-Based Storage** - No database; features stored as JSON files in `.automakeit/` directory
...
Stored in `{projectPath}/.automakeit/`:
