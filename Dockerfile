# AutoMakeIt Multi-Stage Dockerfile
# Single Dockerfile for both server and UI builds
# Usage:
#   docker build --target server -t automakeit-server .
#   docker build --target ui -t automakeit-ui .
...
RUN addgroup -g 1001 -S automakeit && \
    adduser -S automakeit -u 1001
...
RUN mkdir -p /data /projects && chown automakeit:automakeit /data /projects
