/**
 * AutoMakeIt Paths - Utilities for managing automakeit data storage
 *
 * Provides functions to construct paths for:
 * - Project-level data stored in {projectPath}/.automakeit/
...
/**
 * Get the automakeit data directory root for a project
 *
 * All project-specific automakeit data is stored under {projectPath}/.automakeit/
 * This directory is created when needed via ensureAutoMakeItDir().
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit
 */
export function getAutoMakeItDir(projectPath: string): string {
  return path.join(projectPath, '.automakeit');
}
 ...
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/features
 */
export function getFeaturesDir(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'features');
}
 ...
 * @returns Absolute path to {projectPath}/.automakeit/features/{featureId}
 */
 ...
 * @param projectPath - Absolute path to project directory
 * @param featureId - Feature identifier
 * @returns Absolute path to {projectPath}/.automakeit/features/{featureId}/images
 */
 ...
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/board
 */
export function getBoardDir(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'board');
}
 ...
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/images
 */
export function getImagesDir(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'images');
}
 ...
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/context
 */
export function getContextDir(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'context');
}
 ...
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/worktrees
 */
export function getWorktreesDir(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'worktrees');
}
 ...
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/validations
 */
export function getValidationsDir(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'validations');
}
 ...
 * @returns Absolute path to {projectPath}/.automakeit/validations/{issueNumber}
 */
 ...
 * @param projectPath - Absolute path to project directory
 * @param issueNumber - GitHub issue number
 * @returns Absolute path to {projectPath}/.automakeit/validations/{issueNumber}/validation.json
 */
 ...
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/app_spec.txt
 */
export function getAppSpecPath(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'app_spec.txt');
}
 ...
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/active-branches.json
 */
export function getBranchTrackingPath(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'active-branches.json');
}

/**
 * Create the automakeit directory structure for a project if it doesn't exist
 *
 * Creates {projectPath}/.automakeit with all subdirectories recursively.
 * Safe to call multiple times - uses recursive: true.
 *
 * @param projectPath - Absolute path to project directory
 * @returns Promise resolving to the created automakeit directory path
 */
export async function ensureAutoMakeItDir(projectPath: string): Promise<string> {
  const automakeitDir = getAutoMakeItDir(projectPath);
  await secureFs.mkdir(automakeitDir, { recursive: true });
  return automakeitDir;
}
 ...
 *
 * Default locations:
 * - macOS: ~/Library/Application Support/automakeit
 * - Windows: %APPDATA%\automakeit
 * - Linux: ~/.config/automakeit
 *
 ...
 * Located within the project's .automakeit directory.
 *
 * @param projectPath - Absolute path to project directory
 * @returns Absolute path to {projectPath}/.automakeit/settings.json
 */
export function getProjectSettingsPath(projectPath: string): string {
  return path.join(getAutoMakeItDir(projectPath), 'settings.json');
}
