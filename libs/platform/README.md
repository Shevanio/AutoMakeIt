...
```bash
npm install @automakeit/platform
```
...
// Get specific feature directory: /project/.automakeit/features/feature-id
const featureDir = getFeatureDir('/project/path', 'feature-id');

// Get feature images: /project/.automakeit/features/feature-id/images
const imagesDir = getFeatureImagesDir('/project/path', 'feature-id');

// Ensure .automakeit directory exists
await ensureAutoMakeItDir('/project/path');
```
...
```typescript
import { spawnJSONLProcess, spawnProcess } from '@automakeit/platform';
