...
```typescript
import type { Feature, FeatureStatus, PlanningMode, PlanSpec } from '@automakeit/types';
```
...
```typescript
import type { ErrorType, ErrorInfo } from '@automakeit/types';
```
...
```typescript
import type { ImageData, ImageContentBlock } from '@automakeit/types';
```
...
```typescript
import { CLAUDE_MODEL_MAP, DEFAULT_MODELS, type ModelAlias } from '@automakeit/types';
```
...
**IMPORTANT**: This package must NEVER depend on other `@automakeit/*` packages to prevent circular dependencies. All other packages depend on this one, making it the foundation of the dependency tree.
...
1. **Never add dependencies** to other `@automakeit/*` packages in `package.json`
2. **Keep result types here** - For example, `DependencyResolutionResult` should stay in `@automakeit/dependency-resolver`, not be moved here
...
```
@automakeit/types (foundation - no dependencies)
    ↓
@automakeit/utils, @automakeit/platform, etc.
    ↓
