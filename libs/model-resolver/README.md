...
```bash
npm install @automakeit/model-resolver
```
...
import { CLAUDE_MODEL_MAP } from '@automakeit/types';
...
```typescript
import { getEffectiveModel } from '@automakeit/model-resolver';
...
```typescript
import { DEFAULT_MODELS } from '@automakeit/model-resolver';
import { CLAUDE_MODEL_MAP } from '@automakeit/types';
...
- `@automakeit/types` - Model type definitions and constants

## Used By

- `@automakeit/server` - Feature execution, agent chat, enhancement
