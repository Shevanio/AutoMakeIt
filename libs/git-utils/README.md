...
```bash
npm install @automakeit/git-utils
```
...
```typescript
import { isGitRepo } from '@automakeit/git-utils';
...
```typescript
import { parseGitStatus } from '@automakeit/git-utils';
import type { FileStatus } from '@automakeit/git-utils';
...
```typescript
import { listAllFilesInDirectory, generateDiffsForNonGitDirectory } from '@automakeit/git-utils';
...
```typescript
import { isGitRepo, getGitRepositoryDiffs, parseGitStatus } from '@automakeit/git-utils';
...
try {
  ...
  // Errors are logged via @automakeit/utils logger
...
- `@automakeit/types` - FileStatus type definition
- `@automakeit/utils` - Logger utilities

## Used By

- `@automakeit/server` - Git routes, worktree operations, feature context
