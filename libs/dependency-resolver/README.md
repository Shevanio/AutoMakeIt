...
```bash
npm install @automakeit/dependency-resolver
```
...
```typescript
import { getBlockingDependencies } from '@automakeit/dependency-resolver';
...
} from '@automakeit/dependency-resolver';
import type { Feature } from '@automakeit/types';
...
- `@automakeit/types` - Feature type definition

## Used By

- `@automakeit/server` - Auto-mode feature execution
- `@automakeit/ui` - Board view feature ordering
