...
import type { ExecuteOptions } from '@automakeit/types';
import {
  readImageAsBase64,
...
import { PathNotAllowedError } from '@automakeit/platform';
import type { SettingsService } from './settings-service.js';
import {
  getAutoLoadClaudeMdSetting,
  getEnableSandboxModeSetting,
  filterClaudeMdFromContext,
  filterAgentRunnerContext,
...
export class AgentService {
  ...
  async sendMessage({
    ...
    try {
      ...
      // Filter context to only essential files (CLAUDE.md + app_spec.txt) to prevent "Prompt is too long" errors
      // Other context files can be accessed on-demand via Read tool
      const contextFilesPrompt = filterAgentRunnerContext(contextResult, autoLoadClaudeMd);
