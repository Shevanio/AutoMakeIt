 ...
 * - Per-project settings ({projectPath}/.automakeit/settings.json)
 */
 ...
 * - Project settings: per-project overrides in {projectPath}/.automakeit/settings.json
 *
...
export class SettingsService {
   ...
   *
   * @param dataDir - Absolute path to global data directory (e.g., ~/.automakeit)
   */
   ...
   *
   * Reads from {projectPath}/.automakeit/settings.json. If file doesn't exist,
   ...
   *
   * Performs a deep merge on boardBackground. Creates .automakeit directory
   ...
   *
   * @param projectPath - Absolute path to project directory
   * @returns Promise resolving to true if {projectPath}/.automakeit/settings.json exists
   */
  ...
  async migrateFromLocalStorage(localStorageData: {
    'automakeit-storage'?: string;
    'automakeit-setup'?: string;
    'worktree-panel-collapsed'?: string;
    'file-browser-recent-folders'?: string;
    'automakeit:lastProjectDir'?: string;
  }): Promise<{
    success: boolean;
    ...
    try {
      // Parse the main automakeit-storage
      let appState: Record<string, unknown> = {};
      if (localStorageData['automakeit-storage']) {
        try {
          const parsed = JSON.parse(localStorageData['automakeit-storage']);
          appState = parsed.state || parsed;
        } catch (e) {
          errors.push(`Failed to parse automakeit-storage: ${e}`);
        }
      }
      ...
      if (localStorageData['automakeit:lastProjectDir']) {
        globalSettings.lastProjectDir = localStorageData['automakeit:lastProjectDir'];
      }
