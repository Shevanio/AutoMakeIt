...
import type { ExecuteOptions, Feature, PipelineConfig, PipelineStep } from '@automakeit/types';
import {
...
import { resolveWorktreePath } from '@automakeit/git-utils';
import {
  getFeatureDir,
  getAutoMakeItDir,
  getFeaturesDir,
...
export class AutoModeService {
  ...
  async executeFeature(
    ...
    try {
      ...
      if (!feature) {
        throw new Error(
          `Feature "${featureId}" not found in project "${projectPath}". Ensure the feature exists in .automakeit/features/.`
        );
      }
      ...
      // Combine context files prompt with AI profile's system prompt (if any)
      const combinedSystemPrompt = await this.combineSystemPrompts(
        feature,
        contextFilesPrompt,
        projectPath
      );
...
Complete the pipeline step instructions above. Review the previous work and apply the required changes or actions.`;
  ...
  async resumeFeature(projectPath: string, featureId: string, useWorktrees = false): Promise<void> {
    ...
    // Check if context exists in .automakeit directory
    const featureDir = getFeatureDir(projectPath, featureId);
    const contextPath = path.join(featureDir, 'agent-output.md');
  ...
  async followUpFeature(
    ...
    // Combine context files prompt with AI profile's system prompt (if feature exists and has profile)
    const combinedSystemPrompt = feature
      ? await this.combineSystemPrompts(feature, contextFilesPrompt, projectPath)
      : contextFilesPrompt;
...
Address the follow-up instructions above. Review the previous work and make the requested changes or fixes.`;
    ...
    try {
      ...
      const commitMessage = feature
        ? `feat: ${this.extractTitleFromDescription(
            feature.description
          )}\n\nImplemented by AutoMakeIt auto-mode`
  ...
  async contextExists(projectPath: string, featureId: string): Promise<boolean> {
    // Context is stored in .automakeit directory
    const featureDir = getFeatureDir(projectPath, featureId);
    const contextPath = path.join(featureDir, 'agent-output.md');
...
Format your response as a structured markdown document.`;
    ...
    try {
      ...
      // Save analysis to .automakeit directory
      const automakeitDir = getAutoMakeItDir(projectPath);
      const analysisPath = path.join(automakeitDir, 'project-analysis.md');
  ...
  private async loadFeature(projectPath: string, featureId: string): Promise<Feature | null> {
    // Features are stored in .automakeit directory
    const featureDir = getFeatureDir(projectPath, featureId);
...
This helps parse your summary correctly in the output logs.`;
  ...
  /**
   * Combines context files prompt with AI profile's system prompt (if any)
   */
  private async combineSystemPrompts(
    feature: Feature,
    contextFilesPrompt: string | undefined,
    projectPath: string
  ): Promise<string | undefined> {
    // If no profile ID, just return context files prompt
    if (!feature.aiProfileId) {
      return contextFilesPrompt;
    }

    try {
      // Load global settings to get AI profiles
      if (!this.settingsService) {
        logger.warn('Settings service not available, cannot load AI profile');
        return contextFilesPrompt;
      }

      const settings = await this.settingsService.getGlobalSettings();
      const profile = settings.aiProfiles?.find((p: any) => p.id === feature.aiProfileId);

      if (!profile || !profile.systemPrompt) {
        // Profile not found or has no system prompt, return context only
        return contextFilesPrompt;
      }

      // Combine: Profile prompt first, then context files
      const parts: string[] = [];

      if (profile.systemPrompt) {
        parts.push(`# AI Profile: ${profile.name}\n\n${profile.systemPrompt}`);
      }

      if (contextFilesPrompt) {
        parts.push(contextFilesPrompt);
      }

      return parts.length > 0 ? parts.join('\n\n---\n\n') : undefined;
    } catch (error) {
      logger.warn(`Failed to load AI profile ${feature.aiProfileId}:`, error);
      return contextFilesPrompt;
    }
  }
...
`;
    ...
    // Agent output goes to .automakeit directory
