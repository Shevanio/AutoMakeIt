...
async function createAppSpecSymlinkIfNeeded(filePath: string): Promise<void> {
  try {
    ...
    // Check if this is in .automakeit directory
    if (!resolvedPath.includes('.automakeit')) {
      return;
    }

    // Extract project path from .automakeit/app_spec.txt
    const automakeitDir = path.dirname(resolvedPath);
    const projectPath = path.dirname(automakeitDir);
