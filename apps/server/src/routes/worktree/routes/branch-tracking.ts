 ...
 *
 * Tracks active branches in .automakeit so users
 * can switch between branches even after worktrees are removed.
 */

import * as secureFs from '../../../lib/secure-fs.js';
import path from 'path';
import { getBranchTrackingPath, ensureAutoMakeItDir } from '@automakeit/platform';
...
async function saveTrackedBranches(projectPath: string, branches: TrackedBranch[]): Promise<void> {
  ...
  const filePath = path.join(automakeitDir, 'active-branches.json');
