...
import type { ContextFilesResult, ContextFileInfo } from '@automakeit/utils';
import { createLogger } from '@automakeit/utils';
import type { MCPServerConfig, McpServerConfig, PromptCustomization } from '@automakeit/types';
import {
  mergeAutoModePrompts,
  mergeAgentPrompts,
  mergeBacklogPlanPrompts,
  mergeEnhancementPrompts,
} from '@automakeit/prompts';
...
/**
 * Filters context files to include only essential files for Agent Runner
 * (CLAUDE.md and app_spec.txt) to reduce prompt size.
 *
 * This prevents "Prompt is too long" errors by loading only core context.
 * If the agent needs other context files, it can use the Read tool.
 *
 * @param contextResult - Result from loadContextFiles
 * @param autoLoadClaudeMd - Whether SDK auto-loading is enabled
 * @returns Filtered context prompt with only essential files
 */
export function filterAgentRunnerContext(
  contextResult: ContextFilesResult,
  autoLoadClaudeMd: boolean
): string {
  ...
  // Define allowed files (case-insensitive)
  const allowedFiles = ['claude.md', 'app_spec.txt'];

  // Filter to only allowed files
  let filteredFiles = contextResult.files.filter((f) =>
    allowedFiles.includes(f.name.toLowerCase())
  );

  // If autoLoadClaudeMd is enabled, further filter out CLAUDE.md (SDK handles it)
  if (autoLoadClaudeMd) {
    filteredFiles = filteredFiles.filter((f) => f.name.toLowerCase() !== 'claude.md');
  }

  // If no files remain after filtering, return empty string
  if (filteredFiles.length === 0) {
    return '';
  }

  // Rebuild prompt with only essential files
  const formattedFiles = filteredFiles.map((file) => formatContextFileEntry(file));
 ...
 * (Matches the format used in @automakeit/utils/context-loader.ts)
 */
