 ...
 * Stores worktree-specific data in .automakeit/worktrees/:branch/worktree.json
 */
...
function getWorktreeMetadataDir(projectPath: string, branch: string): string {
  ...
  return path.join(projectPath, '.automakeit', 'worktrees', safeBranch);
}
...
export async function readAllWorktreeMetadata(
  ...
  const worktreesDir = path.join(projectPath, '.automakeit', 'worktrees');
