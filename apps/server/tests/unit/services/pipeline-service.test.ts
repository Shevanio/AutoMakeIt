...
import type { PipelineConfig, PipelineStep } from '@automakeit/types';
...
// Mock ensureAutoMakeItDir
vi.mock('@automakeit/platform', () => ({
  ensureAutoMakeItDir: vi.fn(),
}));
...
describe('pipeline-service.ts', () => {
  ...
  describe('getPipelineConfig', () => {
    ...
    it('should read and return existing config', async () => {
      ...
      const configPath = path.join(testProjectDir, '.automakeit', 'pipeline.json');
  ...
  describe('savePipelineConfig', () => {
    it('should save config to file', async () => {
      ...
      vi.mocked(ensureAutoMakeItDir).mockResolvedValue(undefined);
      ...
      expect(ensureAutoMakeItDir).toHaveBeenCalledWith(testProjectDir);
