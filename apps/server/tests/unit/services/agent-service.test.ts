...
// Mock @automakeit/utils with a logger factory
vi.mock('@automakeit/utils', () => {
  const mockLogger = {
    info: vi.fn(),
    error: vi.fn(),
    warn: vi.fn(),
...
vi.mock('@automakeit/utils', async () => {
  const actual = await vi.importActual<typeof import('@automakeit/utils')>('@automakeit/utils');
  return {
    ...actual,
    loadContextFiles: vi.fn(),
    buildPromptWithImages: vi.fn(),
...
describe('agent-service.ts', () => {
  ...
  describe('createSession', () => {
    ...
    it('should validate working directory', async () => {
      ...
      vi.resetModules();
      const { initAllowedPaths } = await import('@automakeit/platform');
      initAllowedPaths();
      ...
      vi.resetModules();
      const { initAllowedPaths: reinit } = await import('@automakeit/platform');
      reinit();
    });
  });
