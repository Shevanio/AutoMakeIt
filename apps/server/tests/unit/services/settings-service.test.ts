...
describe('settings-service.ts', () => {
  ...
  describe('getProjectSettings', () => {
    ...
    it('should read and return existing project settings', async () => {
      ...
      const automakeitDir = path.join(testProjectDir, '.automakeit');
      await fs.mkdir(automakeitDir, { recursive: true });
      const settingsPath = path.join(automakeitDir, 'settings.json');
      await fs.writeFile(settingsPath, JSON.stringify(customSettings, null, 2));
  ...
  describe('updateProjectSettings', () => {
    ...
    it('should create .automakeit directory if it does not exist', async () => {
      ...
      const automakeitDir = path.join(newProjectDir, '.automakeit');
  ...
  describe('migrateFromLocalStorage', () => {
    it('should migrate global settings from localStorage data', async () => {
      const localStorageData = {
        'automakeit-storage': JSON.stringify({
          state: {
            theme: 'light',
            sidebarOpen: false,
            maxConcurrency: 5,
    ...
    it('should handle direct localStorage values', async () => {
      const localStorageData = {
        'automakeit:lastProjectDir': '/path/to/project',
    ...
    it('should handle invalid JSON gracefully', async () => {
      const localStorageData = {
        'automakeit-storage': 'invalid json',
