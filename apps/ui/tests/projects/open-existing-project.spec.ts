 ...
 * 2. The .automakeit directory is initialized if it doesn't exist
...
test.describe('Open Project', () => {
  ...
  test('should open an existing project directory from recent projects', async ({ page }) => {
    ...
    // Verify .automakeit directory was created (initialized for the first time)
    // Use polling since file creation may be async
    const automakeitDir = path.join(projectPath, '.automakeit');
    await expect(async () => {
      expect(fs.existsSync(automakeitDir)).toBe(true);
    }).toPass({ timeout: 10000 });

    // Verify the required structure was created by initializeProject:
    // - .automakeit/categories.json
    // - .automakeit/features directory
    // - .automakeit/context directory
    // Note: app_spec.txt is NOT created automatically for existing projects
    const categoriesPath = path.join(automakeitDir, 'categories.json');
    await expect(async () => {
      expect(fs.existsSync(categoriesPath)).toBe(true);
    }).toPass({ timeout: 10000 });

    // Verify subdirectories were created
    expect(fs.existsSync(path.join(automakeitDir, 'features'))).toBe(true);
    expect(fs.existsSync(path.join(automakeitDir, 'context'))).toBe(true);
