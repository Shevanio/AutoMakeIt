...
export async function createTestGitRepo(tempDir: string): Promise<TestRepo> {
  ...
  // Create .automakeit directories
  const automakeitDir = path.join(tmpDir, '.automakeit');
  const featuresDir = path.join(automakeitDir, 'features');
  fs.mkdirSync(featuresDir, { recursive: true });

  // Create empty categories.json to avoid ENOENT errors in tests
  fs.writeFileSync(path.join(automakeitDir, 'categories.json'), '[]');
...
export function createTestFeature(
  ...
  const featuresDir = path.join(repoPath, '.automakeit', 'features');
...
export function readTestFeature(repoPath: string, featureId: string): FeatureData | null {
  const featureFilePath = path.join(repoPath, '.automakeit', 'features', featureId, 'feature.json');
...
export async function setupProjectWithPath(page: Page, projectPath: string): Promise<void> {
  await page.addInitScript((pathArg: string) => {
    ...
    localStorage.setItem('automakeit-storage', JSON.stringify(mockState));
    ...
    localStorage.setItem('automakeit-setup', JSON.stringify(setupState));
  }, projectPath);
}
