...
export async function setupWelcomeView(
  ...
  await page.addInitScript(
    ({
      ...
      localStorage.setItem('automakeit-storage', JSON.stringify(appState));
      ...
      if (opts?.workspaceDir) {
        localStorage.setItem('automakeit:lastProjectDir', opts.workspaceDir);
      }
    },
  ...
  await page.addInitScript(
    ({ featureId, contextContent }: { featureId: string; contextContent: string }) => {
      ...
      (window as any).__mockContextFile = {
        featureId,
        path: `/mock/test-project/.automakeit/features/${featureId}/agent-output.md`,
...
export async function setupFirstRun(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    localStorage.removeItem('automakeit-setup');
    localStorage.removeItem('automakeit-storage');
