...
test.describe('Agent Chat Session', () => {
  ...
  test.beforeAll(async () => {
    ...
    fs.mkdirSync(automakeitDir, { recursive: true });
    fs.mkdirSync(path.join(automakeitDir, 'features'), { recursive: true });
    fs.mkdirSync(path.join(automakeitDir, 'context'), { recursive: true });
    fs.mkdirSync(path.join(automakeitDir, 'sessions'), { recursive: true });

    fs.writeFileSync(
      path.join(automakeitDir, 'categories.json'),
      JSON.stringify({ categories: [] }, null, 2)
    );

    fs.writeFileSync(
      path.join(automakeitDir, 'app_spec.txt'),
