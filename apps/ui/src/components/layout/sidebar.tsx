...
import { initializeProject, hasAppSpec, hasAutoMakeItDir } from '@/lib/project-init';
...
export function Sidebar() {
  ...
  const handleOpenFolder = useCallback(async () => {
    ...
    if (!result.canceled && result.filePaths[0]) {
      ...
      try {
        // Check if this is a brand new project (no .automakeit directory)
        const hadAutoMakeItDir = await hasAutoMakeItDir(path);
        ...
        if (!hadAutoMakeItDir && !specExists) {
          ...
          toast.success(initResult.isNewProject ? 'Project initialized' : 'Project updated', {
            description: `Set up ${initResult.createdFiles.length} file(s) in .automakeit`,
          });
        } else {
          toast.success('Project opened', {
