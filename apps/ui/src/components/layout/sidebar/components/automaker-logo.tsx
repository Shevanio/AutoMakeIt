...
interface AutoMakeItLogoProps {
  sidebarOpen: boolean;
  navigate: (opts: NavigateOptions) => void;
}

export function AutoMakeItLogo({ sidebarOpen, navigate }: AutoMakeItLogoProps) {
  return (
    <div
      className={cn(
      ...
      {!sidebarOpen ? (
        ...
        <div className={cn('flex items-center gap-1', 'hidden lg:flex')}>
          ...
          <span className="font-bold text-foreground text-[1.7rem] tracking-tight leading-none translate-y-[-2px]">
            automakeit<span className="text-brand-500">.</span>
          </span>
        </div>
      )}
    </div>
