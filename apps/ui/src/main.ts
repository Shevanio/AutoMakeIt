...
import { findNodeExecutable, buildEnhancedPath } from '@automakeit/platform';
...
// Window bounds interface (matches @automakeit/types WindowBounds)
...
app.whenReady().then(async () => {
  // Ensure userData path is consistent across dev/prod so files land in AutoMakeIt dir
  try {
    const desiredUserDataPath = path.join(app.getPath('appData'), 'AutoMakeIt');
