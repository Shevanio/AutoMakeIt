...
export interface AppState {
  ...
  boardBackgroundByProject: Record<
    string,
    {
      imagePath: string | null; // Path to background image in .automakeit directory
...
const DEFAULT_AI_PROFILES: AIProfile[] = [
  ...
  // Specialized agent profiles (from Spec Editor multi-agent system)
  {
    id: 'profile-frontend-specialist',
    name: 'Frontend Specialist',
    description:
      'Expert in React, TypeScript, UI/UX, components, state management, and user interactions. Best for frontend-focused features.',
    model: 'sonnet',
    thinkingLevel: 'medium',
    provider: 'claude',
    isBuiltIn: true,
  ...
  {
    id: 'profile-backend-specialist',
    name: 'Backend Specialist',
    description:
      'Expert in Node.js, Express, API design, business logic, and data processing. Best for backend/API features.',
    model: 'sonnet',
    thinkingLevel: 'medium',
    provider: 'claude',
    isBuiltIn: true,
  ...
  {
    id: 'profile-database-specialist',
    name: 'Database Specialist',
    description:
      'Expert in schema design, migrations, queries, indexes, and data integrity. Best for data-heavy features.',
    model: 'sonnet',
    thinkingLevel: 'medium',
    provider: 'claude',
    isBuiltIn: true,
  ...
  {
    id: 'profile-security-specialist',
    name: 'Security Specialist',
    description:
      'Expert in authentication, authorization, data protection, and vulnerability prevention. Best for security-critical features.',
    model: 'opus',
    thinkingLevel: 'high',
    provider: 'claude',
    isBuiltIn: true,
  ...
  {
    id: 'profile-testing-specialist',
    name: 'Testing Specialist',
    description:
      'Expert in test strategy, unit tests, integration tests, E2E tests, and edge cases. Best for test-driven features.',
    model: 'sonnet',
    thinkingLevel: 'medium',
    provider: 'claude',
    isBuiltIn: true,
  ...
  {
    id: 'profile-devops-specialist',
    name: 'DevOps Specialist',
    description:
      'Expert in deployment, CI/CD, monitoring, infrastructure, and scalability. Best for deployment and operations features.',
    model: 'sonnet',
    thinkingLevel: 'medium',
    provider: 'claude',
    isBuiltIn: true,
  ...
  persist(
          ...
          messages: [
            {
              ...
              role: 'assistant',
              content:
                "Hello! I'm the AutoMakeIt Agent. I can help you build software autonomously. What would you like to create today?",
              timestamp: now,
            },
          ],
          createdAt: now,
    ...
    }),
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        // - Add new specialized built-in profiles (Frontend, Backend, Database, Security, Testing, DevOps)
        if (version <= 2) {
          const defaultProfileIds = new Set(DEFAULT_AI_PROFILES.map((p) => p.id));
          const existingProfileIds = new Set((state.aiProfiles || []).map((p) => p.id));

          // Find new built-in profiles that don't exist in persisted state
          const newBuiltInProfiles = DEFAULT_AI_PROFILES.filter(
            (p) => p.isBuiltIn && !existingProfileIds.has(p.id)
          );

          // Keep user-created profiles
          const userProfiles = (state.aiProfiles || []).filter((p) => !p.isBuiltIn);

          // Update existing built-in profiles with latest definitions
          const updatedBuiltInProfiles = DEFAULT_AI_PROFILES.filter(
            (p) => p.isBuiltIn && existingProfileIds.has(p.id)
          );

          state.aiProfiles = [...updatedBuiltInProfiles, ...newBuiltInProfiles, ...userProfiles];
        }

        // Migration from version 3 to version 4:
        // - Update built-in profile icons (Palette, Settings, Database, Lock, TestTube)
        if (version <= 3) {
          const defaultProfileIds = new Set(DEFAULT_AI_PROFILES.map((p) => p.id));

          // Update all built-in profiles with latest definitions (including new icons)
          const updatedBuiltInProfiles = DEFAULT_AI_PROFILES.filter((p) => p.isBuiltIn);

          // Keep user-created profiles
          const userProfiles = (state.aiProfiles || []).filter((p) => !p.isBuiltIn);

          state.aiProfiles = [...updatedBuiltInProfiles, ...userProfiles];
        }
