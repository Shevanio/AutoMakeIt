 ...
 * 4. Maintains automakeit-storage in localStorage as fast cache for Zustand
 *
 ...
 * The automakeit-storage key is handled specially as it's still used by Zustand.
 */
const LOCALSTORAGE_KEYS = [
  ...
  'automakeit:lastProjectDir',
] as const;

/**
 * localStorage keys to remove after successful migration
 *
 * automakeit-storage is intentionally NOT in this list because Zustand still uses it
...
const KEYS_TO_CLEAR_AFTER_MIGRATION = [
  ...
  'automakeit_projects',
  'automakeit_current_project',
  'automakeit_trashed_projects',
] as const;
  ...
  useEffect(() => {
    ...
    async function checkAndMigrate() {
      ...
      try {
        ...
        const automakeitStorage = getItem('automakeit-storage');
        if (!automakeitStorage) {
        ...
        if (result.success) {
          ...
          // Clear old localStorage keys (but keep automakeit-storage for Zustand)
...
export async function syncSettingsToServer(): Promise<boolean> {
  try {
    ...
    const parsed = JSON.parse(automakeitStorage);
 ...
 * to be written to {projectPath}/.automakeit/settings.json.
 *
