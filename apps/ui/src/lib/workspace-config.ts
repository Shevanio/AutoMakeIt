...
const LAST_PROJECT_DIR_KEY = 'automakeit:lastProjectDir';

/**
 * Gets the default Documents/AutoMakeIt directory path
 * @returns Promise resolving to Documents/AutoMakeIt path, or null if unavailable
 */
async function getDefaultDocumentsPath(): Promise<string | null> {
  try {
    const api = getElectronAPI();
    const documentsPath = await api.getPath('documents');
    return path.join(documentsPath, 'AutoMakeIt');
  } catch (error) {
 ...
 * 3. Documents/AutoMakeIt (if ALLOWED_ROOT_DIRECTORY is not set)
...
export async function getDefaultWorkspaceDirectory(): Promise<string | null> {
  try {
    ...
    if (result.success) {
      ...
      // Try to get Documents/AutoMakeIt
