 ...
 *
 * Handles the setup of the .automakeit directory structure when opening
...
/**
 * Required files and directories in the .automakeit directory
...
const REQUIRED_STRUCTURE: {
  ...
  directories: ['.automakeit', '.automakeit/context', '.automakeit/features', '.automakeit/images'],
  files: {
    '.automakeit/categories.json': '[]',
  },
};

/**
 * Initializes the .automakeit directory structure for a project
 *
...
/**
 * Checks if a project has the required .automakeit structure
 *
 * @param projectPath - The root path of the project
...
export async function hasAppSpec(projectPath: string): Promise<boolean> {
  const api = getElectronAPI();
  try {
    const fullPath = `${projectPath}/.automakeit/app_spec.txt`;
...
/**
 * Checks if the .automakeit directory exists for a project
 *
 * @param projectPath - The root path of the project
 * @returns true if .automakeit directory exists
 */
export async function hasAutoMakeItDir(projectPath: string): Promise<boolean> {
  const api = getElectronAPI();
  try {
    const fullPath = `${projectPath}/.automakeit`;
    return await api.exists(fullPath);
  } catch (error) {
    console.error('[project-init] Error checking .automakeit dir:', error);
    return false;
  }
}
