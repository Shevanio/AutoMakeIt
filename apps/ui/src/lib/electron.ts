...
// Local storage keys
const STORAGE_KEYS = {
  PROJECTS: 'automakeit_projects',
  CURRENT_PROJECT: 'automakeit_current_project',
  TRASHED_PROJECTS: 'automakeit_trashed_projects',
} as const;
...
const getMockElectronAPI = (): ElectronAPI => {
  return {
    ...
    readFile: async (filePath: string) => {
      ...
      // Note: Features are now stored in .automakeit/features/{id}/feature.json
      ...
      if (filePath.includes('.automakeit/features/')) {
        if (mockFileSystem[filePath] !== undefined) {
          return { success: true, content: mockFileSystem[filePath] };
        }
    ...
    readdir: async (dirPath: string) => {
      ...
      if (dirPath) {
        ...
        if (dirPath.includes('.automakeit/context')) {
        ...
        if (
          ...
          return {
            ...
            entries: [
              ...
              { name: '.automakeit', isDirectory: true, isFile: false },
    ...
    exists: async (filePath: string) => {
      ...
      // Note: Features are now stored in .automakeit/features/{id}/feature.json
      if (filePath.endsWith('app_spec.txt') && !filePath.includes('.automakeit')) {
        return true;
      }
      return false;
    },
    ...
    saveImageToTemp: async (
      ...
      const tempFilePath = projectPath
        ? `${projectPath}/.automakeit/images/${timestamp}_${safeName}`
        : `/tmp/automakeit-images/${timestamp}_${safeName}`;
...
function createMockAutoModeAPI(): AutoModeAPI {
  return {
    ...
    contextExists: async (projectPath: string, featureId: string) => {
      ...
      const exists =
        mockFileSystem[`${projectPath}/.automakeit/features/${featureId}/agent-output.md`] !==
        undefined;
    ...
    analyzeProject: async (projectPath: string) => {
      ...
      mockFileSystem[`${projectPath}/.automakeit/app_spec.txt`] = `<project_specification>
  <project_name>Demo Project</project_name>

  <overview>
    A demo project analyzed by the AutoMakeIt AI agent.
  </overview>
...
async function simulateAutoModeLoop(projectPath: string, featureId: string) {
  ...
  const contextFilePath = `${projectPath}/.automakeit/features/${featureId}/agent-output.md`;
...
function createMockFeaturesAPI(): FeaturesAPI {
  ...
  return {
    getAll: async (projectPath: string) => {
      ...
      const featuresDir = `${projectPath}/.automakeit/features`;
    ...
    get: async (projectPath: string, featureId: string) => {
      ...
      const featurePath = `${projectPath}/.automakeit/features/${featureId}/feature.json`;
    ...
    create: async (projectPath: string, feature: Feature) => {
      ...
      const featurePath = `${projectPath}/.automakeit/features/${feature.id}/feature.json`;
    ...
    delete: async (projectPath: string, featureId: string) => {
      ...
      const agentOutputPath = `${projectPath}/.automakeit/features/${featureId}/agent-output.md`;
